...
M = 20    # c boundary for M set
e = 15    # break condition for distance from target position
k1 = 10.8#10.8 # control gain for rotational motion
k2 = 0.05#0.1 # control gain for x1 in translational motion
k3 = 0.05#0.1 # control gain for x2 in translational motion
...
def optimize_quadratic(vmin,vmax,zmin,zmax,y,w):
    ...
    try:
        maximum1 = prob1.solve()
    except Exception as e:
        print(e)

    v = cp.Variable()
    z = cp.Variable()
    prob2 = cp.Problem(cp.Maximize(v-z),
                        [cp.square(v-z)   <= y-w , 
                        z >= zmin , z <= zmax, 
                        v >= vmin , v <= vmax ])
    
    try:
        maximum2 = prob2.solve()
    except Exception as e:
        print(e)
...
def simulation():
    ...
    x3in = math.atan2((x2-x2t),(x1-x1t)) + math.pi + 0.01
    est_set = [[x1,x2,x3]]
    ...
    while (d>e):
        ...
        m = abs(df) * (math.sqrt((x1-x1t)**2 + (x2-x2t)**2))
        if m < M :
            if motion != 0 : 
                motion = 0
                changes_in_m +=1 
